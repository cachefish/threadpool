...
struct threadpool_t{
        ...
        pthread_cond_t queue_not_full;            //当任务队列满时，添加任务的线程阻塞，等待此条件变化
        ...
        int queue_max_size;     //task_queue队列可容纳的任务数上限

        int shutdown;                   //标志位，线程池使用状态,true/false
...
threadpool_t*threadpool_create(int min_thr_num,int max_thr_num,int queue_max_size)
{
    ...
    /*      pthread_mutex_t lock;                               //用于锁住该结构体
        ...
        int queue_front;            //task_queue队头下标
        int queue_rear;             //task_queue队尾下标
        int queue_szie;               //task_queue队中实际任务数
        int queue_max_size;     //task_queue队列可容纳的任务数上限
        ...
        for(i=0;i<min_thr_num;i++){
            ...
            printf("start thread  0x%x...",(unsigned int)pool->threads[i]);
        }
...
void *threadpool_thread(void *threadpool)
{
        ...
        while(true){
            ...
            //queue->size==0说明没有任务 调wait阻塞在条件变量上，若有任务，跳过该while
            while((pool->queue_max_size==0)&&(!pool->shutdown)){
